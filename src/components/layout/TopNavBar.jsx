...
export default function TopNavBar({ links = [], transparent = false, showAuth = false }) {
  ...
  return (
    <nav className={`fixed top-0 z-50 w-full h-16 flex justify-between items-center px-8 ${transparent ? 'bg-surface-container-low/80' : 'bg-surface-container-low/80'} backdrop-blur-xl shadow-2xl shadow-black/30`}>
      ...
      <div className="flex items-center gap-4">
        {showAuth ? (
          // Show profile and notifications for authenticated users
          <>
          ...
          // Show login button for public pages
          <Link
            to="/login"
            className="px-6 py-2.5 bg-gradient-to-r from-primary to-on-primary-container text-on-primary font-semibold rounded-lg hover:shadow-lg hover:shadow-primary/30 transition-all hover:scale-105 active:scale-95 flex items-center gap-2"
          >
